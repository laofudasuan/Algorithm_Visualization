...
double mo(C_TYPE a){return sqrt(mo2(a));}
Point norm(C_TYPE(Point)a){return a/mo(a);}
Face::Face(C_TYPE(Point)a,C_TYPE(Point)b,C_TYPE(Point)c):normal(norm(cross(b-a,c-a))),c(normal*a){}
...
void Hull3D::clearAll(){
  FOR(i,0,n-1){
    cp[i].made.clear();cp[i].see.clear();
  }
}
int Hull3D::inHull(Point p,CPoint *s){
  FOR_EACH(i,s->made){
    if(faceDis((*i)->f,p)>Eps){
      if((*i)->killed)return inHull(p,(*i)->killed);
      else return 0;
   }
  }
  return 1;
...
void Hull3D::build(vector<Point>points){
  ...
  FOR(i,3,n-1)insertPoint(a[i]);
}
vector<Face>Hull3D::getAllFace(){
  Face<Facr>fs;
  for(auto p:&cp)
    for(auto &f:p.make)
      if(!k->killed)fs.push_back(f->k);
  return fs;
}
Point Hull3fD::getCenter{
  Point c(0,0,0);
  for(auto &p:cp)
    c=c+p.p;
  return c/cp.size();
...
void work(){
	...
	points.clear();
	for(auto &f:fs)
		point.push_back(faceToPoint(f,center));
	hull2.build(points);
	int q,i=0;
	q=rd();
	whie(i++,q--) {
		...
		int ans;
		if(q==1) ans=hull.inHull(Point(a,b,c)/(a+b+c+d));
		else {
			Face f;f.normal=Point(a-d,b-d,c-d);
			f.c=-d;
			if(!seeFace(f,center)){
