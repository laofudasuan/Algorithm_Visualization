...
import { AdjacencyListAlgorithm } from './AdjacencyListAlgorithm.jsx'

const AdjacencyListMotion = forwardRef(({ graphData }, ref) => {
  const animationCanvasRef = useRef(null)
  const adjacencyListRefs = useRef({})
  ...
  const startExecution = async () => {
    ...
    Object.values(adjacencyListRefs.current).forEach(ref => { if (ref && ref.clear) ref.clear() })
    try {
      const algorithmInstance = AdjacencyListAlgorithm({ graphData, graphAdjList: {}, delayMs, animationCanvasRef, adjacencyListRefs })
      await algorithmInstance.execute()
    } catch (e) {
      console.error('AdjacencyList执行过程中出错:', e)
    } finally {
      setIsPlaying(false)
    }
  }
  ...
  return (
    <div className="relative min-h-screen">
      <h2 className="fixed top-4 left-4 text-2xl font-bold text-gray-800 z-10">邻接表</h2>
      <div className="fixed inset-0 flex items-center justify-center p-4 overflow-auto">
        {graphData ? (
          <div className="flex gap-4 w-full max-w-6xl">
            <div className="flex-1">
              <GraphCanvas ref={animationCanvasRef} width={600} height={500} graphData={graphData} isLoading={false} />
            </div>
              ...
              <div className="grid grid-cols-1 gap-4 overflow-y-auto max-h-[500px]">
                {graphData.nodes && graphData.nodes.map(node => (
                  <div key={node.id} className="flex items-center">
                    ...
                    <LinkedListVisualization ref={el => adjacencyListRefs.current[node.id] = el} radius={30} maxSize={graphData.nodes?.length || 0} />
                  </div>
                ))}
              </div>
            </div>
...
AdjacencyListMotion.displayName = 'AdjacencyListMotion'

export default AdjacencyListMotion
