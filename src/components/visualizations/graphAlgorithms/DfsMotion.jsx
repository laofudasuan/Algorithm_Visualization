...
import { DFSAlgorithm } from './DFSAlgorithm.jsx'

const DfsMotion = forwardRef(({ graphData }, ref) => {
  ...
  const startExecution = async () => {
    ...
    if (stackVizRef.current) stackVizRef.current.clear()
    try {
      const algorithmInstance = DFSAlgorithm({ graphData, graphAdjList, delayMs, animationCanvasRef, stackVizRef, setVisitedNodes })
      await algorithmInstance.execute()
    } catch (e) {
      console.error('DFS执行过程中出错:', e)
    } finally {
      setIsPlaying(false)
    }
  }
  ...
  return (
    <div className="relative min-h-screen">
      <h2 className="fixed top-4 left-4 text-2xl font-bold text-gray-800 z-10">深度优先搜索(DFS)</h2>
      <div className="fixed bottom-4 left-0 right-0 z-8">
        <div className="container mx-auto">
          ...
          <div className="flex justify-center">
            <StackVisualization ref={stackVizRef} height={50} maxSize={10} />
          </div>
        </div>
      </div>
...
DfsMotion.displayName = 'DfsMotion'

export default DfsMotion
